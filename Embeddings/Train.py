...
from Embeddings.Parameters import MARGIN
...
x = T.fvector('x')
mse = function([x], T.mul(T.sum(T.mul(x, x)), 0.5))
...
def build_asts(dataset_dir):
    ...
    for file in files:
        try:
            ...
            continue
        data_ast.append(ast)
    ast_file = open(dataset_dir + 'ast_file', mode='wb')
...
def evaluate(positive: Network, negative: Network, training_token: Token, params: Parameters):
    ...
    pos_target = params.embeddings[training_token.token_index]
...
def main():
    ...
    for file_ast in data_ast:
        for ast in prepare_ast(file_ast):
            ...
            positive = construct(ast, params)

            def rand_token():
                return token_map.keys()[np.random.randint(0, len(token_map))]

            def create_negative(token_index):
                current = ast[token_index]
                new_token = rand_token()
                while current == new_token:
                    new_token = rand_token()
                sample = copy(ast)
                sample[token_index] = new_token
                return construct(sample, params)

            negative = [create_negative(i) for i in np.random.random_integers(0, len(ast) - 1, size=10)]

            for neg in negative:
                err = evaluate(positive, neg, training_token, params)
                print(err)
